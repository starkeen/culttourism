...
class SitemapCommand extends CrontabCommand
{
    ...
    public function run(): void
    {
        ...
        $this->db->sql = "SELECT md_url FROM modules WHERE md_active = '1' AND md_robots = 'index, follow'";
        $this->db->exec();
        ...
        $this->db->sql = "SELECT u.url, DATE_FORMAT(c.pc_lastup_date, '%Y-%m-%dT%H:%i:%s+00:00') dateup
            FROM region_url u
        ...
        $this->db->sql = "SELECT concat(u.url, '/', p.pt_slugline, '.html') url,
                                 DATE_FORMAT(p.pt_lastup_date, '%Y-%m-%dT%H:%i:%s+00:00') dateup
            FROM pagepoints p
