...
a)});$("#type_selector tr").live("click",function(){var a=this.id.split("_"),g=$("#pt_id").val();$.post(b+"ajax/pointtype/savetype/?pid="+$("#pt_id").val(),{pid:g,ntype:a[1]},function(i){$("img#type_"+g+".point_typer").attr("src","../../../img/points/32/"+i);$.modal.close()})});$("a.check_all").click(function(){$("input.export_check").attr("checked","true");return false});$("a.check_not").click(function(){$("input.export_check").removeAttr("checked");return false});$("a.export_about").click(function(){showWindByURL(b+
"ajax/page/gps/",{});return false});$("#captchahelp").click(function(){stamp=new Date;$("#norobotpic").attr("src","../../../feedback/getcapt/"+stamp.getTime());return false});$(".point_latlon").live("click",function(){var a={pid:this.id.split("_")[1]};showWindByURL(b+"ajax/point/getformGPS/",a);return false});$("#pt_latlon_handler input.dosave").live("click",function(){$.post(b+"ajax/point/saveformGPS/?pid="+$("#obj_id").val(),{pt_lat:$("#obj_lat").val(),pt_lon:$("#obj_lon").val(),pt_zoom:$("#obj_zoom").val()},
function(a){if(a){$("#gps_"+$("#obj_id").val()).text(a);$.modal.close()}})});$("#pt_latlon_handler input.doesc").live("click",function(){$.modal.close()});$("#citymap_finder").click(function(){var a={cid:document.location.search.split("city_id=")[1]};showWindByURL(b+"ajax/city/getformGPS/",a);return false});$("#pc_latlon_handler input.dosave").live("click",function(){$.post(b+"ajax/city/saveformGPS/?cid="+$("#city_id").val(),{pc_lat:$("#city_lat").val(),pc_lon:$("#city_lon").val(),pc_zoom:$("#city_zoom").val()},
function(a){if(a){$("#pc_latitude").val($("#city_lat").val());$("#pc_longitude").val($("#city_lon").val());$.modal.close()}})});$("#pc_latlon_handler input.doesc").live("click",function(){$.modal.close()});$("a.typefilterlink").live("click",function(){var a=this.href.split("#")[1].split("_")[1];if(a!="all"){$("#whatseelist tr").hide();$("#whatseelist tr.obj_type_"+a).show()}else $("#whatseelist tr").show();return false});$("a.typefilterlink0").live("click",function(){var a=this.href.split("#")[1].split("_")[1];
if(a!="all"){$("#whatservlist tr").hide();$("#whatservlist tr.obj_type_"+a).show()}else $("#whatservlist tr").show();return false});$(".blog_entry_edit").live("click",function(){var a=this.id.split("_");showWindByURL(b+"ajax/blog/editform/",{brid:a[2]});$("#eblog_text").live("mouseover",function(){$("#eblog_text").die("mouseover");$("#eblog_text").ckeditor(function(){},{customConfig:"../../../../config/config.cke.js"});$("#eblog_date").datepicker({dateFormat:"dd.mm.yy"})});$("#eblog_date").live("click",
function(){$(this).datepicker({dateFormat:"dd.mm.yy"})});return false});$("#blog_entry_add").live("click",function(){showWindByURL(b+"ajax/blog/addform/",null);$("#eblog_text").live("mouseover",function(){$("#eblog_text").die("mouseover");$("#eblog_text").ckeditor(function(){},{customConfig:"../../../../config/config.cke.js"});$("#eblog_date").datepicker({dateFormat:"dd.mm.yy"})});$("#eblog_date").live("click",function(){$(this).datepicker({dateFormat:"dd.mm.yy"})});return false});$(".blog_entry_delete").live("click",
function(){var a=this.id.split("_");confirm('\u0414\u0435\u0439\u0441\u0442\u0432\u0438\u0442\u0435\u043b\u044c\u043d\u043e \u0443\u0434\u0430\u043b\u0438\u0442\u044c \u0437\u0430\u043f\u0438\u0441\u044c "'+$(this).parent("h2").children("a").text()+'"?')&&$.post(b+"ajax/blog/delentry/?bid="+a[2],{brid:a[2]},function(g){if(g)document.location="../../blog/"});return false})});
