    ...
    function CodeMirror(place, givenOptions)
    {
      ...
      // This mess creates the base DOM structure for the editor.
        ...
        if (place.appendChild) {
            place.appendChild(wrapper); } else {
            place(wrapper);
            }
      ...
      // trying to access activeElement before onload
        var hasFocus; try {
            hasFocus = (document.activeElement == input); } catch (e) {
            }
      ...
      // updated afterwards.
            var instance = wrapper.CodeMirror = {
                ...
                focus: function () {
                    window.focus(); focusInput(); onFocus(); fastPoll();},
                setOption: function (option, value) {
                    ...
                    if (option == "mode" || option == "indentUnit") {
                        ...
                        onBlur(); input.blur();} else if (option == "readOnly" && !value) {
                        resetInput(true);} else if (option == "theme") {
                            themeChanged();
            ...
            if (options.electricChars && mode.electricChars && options.smartIndent && !options.readOnly) {
                if (mode.electricChars.indexOf(ch) > -1) {
                    setTimeout(operation(function () {
                        indentLine(sel.to.line, "smart");}), 75);
                }
            }
      ...
      // Afterwards, set the selection to selFrom, selTo.
        ...
        function fastPoll()
        {
            ...
            function p()
            {
                ...
                if (!changed && !missed) {
                    missed = true; poll.set(60, p);} else {
      ...
      // supported or compatible enough yet to rely on.)
        ...
        function scrollIntoView(x1, y1, x2, y2)
        {
            ...
            if (y1 < screentop) {
                scroller.scrollTop = Math.max(0, y1); scrolled = true;} else if (y2 > screentop + screen) {
                          scroller.scrollTop = y2 - screen; scrolled = true;}
      ...
      // Coords must be lineSpace-local
        ...
        function posFromMouse(e, liberal)
        {
          ...
          // Fails unpredictably on IE[67] when mouse is dragged around quickly.
            try {
                x = e.clientX; y = e.clientY; } catch (e) {
                          return null; }
      ...
      // batched and then all combined and executed at once.
        ...
        function compoundChange(f)
        {
            ...
            try {
                return f(); } finally { history.endCompound(); }
        }
  ...
  // Known modes, by name and by MIME
    ...
    var commands = CodeMirror.commands = {
        ...
        deleteLine: function (cm) {
            var l = cm.getCursor().line; cm.replaceRange("", {line: l, ch: 0}, {line: l});},
        undo: function (cm) {
            cm.undo();},
        redo: function (cm) {
            cm.redo();},
    ...
    function eltOffset(node, screen)
    {
        ...
        for (var n = node; n; n = n.offsetParent) {
          ...
          // Firefox reports weird inverted offsets when the body has a border.
            if (n == bod) {
                x += Math.abs(ol); y += Math.abs(ot); } else {
                          x += ol, y += ot; }
  ...
  // Use the faster and saner getBoundingClientRect method when possible.
    if (document.documentElement.getBoundingClientRect != null) {
        eltOffset = function (node, screen) {
            ...
            try {
                var box = node.getBoundingClientRect(); box = { top: box.top, left: box.left }; } catch (e) {
                box = {top: 0, left: 0}; }
                if (!screen) {
